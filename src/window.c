...
void activate(GtkApplication *app, gpointer user_data)
{
  ...
  {
    GtkWidget *square_image;
    Square* sq = (Square*)malloc(sizeof(Square));

    if (NULL != sq && NULL != square_image)
    {
      square_image = gtk_image_new_from_file(IMAGE_PATH_SQUARE);
      initialize_square(sq, square_image);
      draw_board(sq, box);
    }
  }
